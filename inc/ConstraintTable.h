...
{
public:
	int length_min = 0;	// the agent must reach its goal after length_min
	int length_max = MAX_TIMESTEP; // the agent must reach its goal before length_max
	int goal_location;
	...
	int getNumOfTargetConflictsForStep(size_t curr_id, size_t next_id, int next_timestep) const;
	int getNumOfConflictingAgentsForStep(size_t curr_id, size_t next_id, int next_timestep) const;
	...
	void buildCATwithSemantic(int agent, const vector<Path*>& paths, size_t cat_size); // build the conflict avoidance table
	void buildCATwithNumber(int agent, const vector<Path*>& paths, size_t cat_size, bool excludeTarget); // build the conflict avoidance table
...
private:
	...
	vector<vector<bool> > catTarget; // conflict avoidance table for target
	vector<vector<int>> catNum_; // conflict avoidance table
};
