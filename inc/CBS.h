...
enum high_level_solver_type { ASTAR, ASTAREPS, NEW, EES, COASTAR };
...
public:
	...
	void saveSummary(const string &fileName, const string &instanceName) const;
  ...
  void savePathsTable(const string &fileName) const; // write the paths to a file
...
private: // CBS only, cannot be used by ECBS
	...
	pairing_heap< CBSNode*, compare<CBSNode::compare_node_by_b> > bcoa_list; // this is ued for both ECBS and EES
