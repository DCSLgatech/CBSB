...
int main(int argc, char** argv)
{
	...
	desc.add_options()
		...
		("lowLevelSolver", po::value<string>()->default_value("A*"), "the low-level solver(A*,A*eps,COA*)")
	...
	else if (vm["highLevelSolver"].as<string>() == "COA*")
		s = high_level_solver_type::COASTAR;
	else
	{
  ...
  // initialize the solver
	if (vm["lowLevelSolver"].as<string>() == "A*")
	{
	...
	else if (vm["lowLevelSolver"].as<string>() == "A*eps")
	{
	...
	else if (vm["lowLevelSolver"].as<string>() == "COA*")
	{
		...
		cbsb.setSavingStats(vm["stats"].as<bool>());
		cbsb.setHighLevelSolver(s, vm["suboptimality"].as<double>());
		//////////////////////////////////////////////////////////////////////
		// run
		double runtime = 0;
		int lowerbound = 0;
		...
		if (vm.count("output"))
				cbsb.saveResults(vm["output"].as<string>(), vm["agents"].as<string>());
		...
		if (vm["stats"].as<bool>())
				cbsb.saveStats(vm["output"].as<string>(), vm["agents"].as<string>());
