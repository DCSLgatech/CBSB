	...
	const vector<Path*>& paths, int agent, int lowerbound, double w)
{
  ...
  lowerbound =  max(holding_time, lowerbound);
	...
	{
		updateFocalList(); // update FOCAL if min f-val increased, also update min_f_val
		...
		for (int next_location : next_locations)
		{
			...
			// compute the number of conflicts so far found to this path.
			...
			// if (excludeTarget_ && constraint_table.getNumOfTargetConflictsForStep(curr->location, next_location, next_timestep)>0)
			// {
			// 	next_internal_conflicts++;
			// }
			// else if (excludeCorridor_ && instance.getDegree(curr->location)==2 && constraint_table.getNumOfConflictsForStep(curr->location, next_location, next_timestep)>0)
			// {
			// 	next_internal_conflicts++;
			// }
			...
			// int next_internal_conflicts = curr->num_of_conflicts +
			// 	constraint_table.getNumOfConflictingAgentsForStep(curr->location, next_location, next_timestep);
					...
					existing_next->num_of_conflicts > next->num_of_conflicts)) // or it remains the same but there's fewer conflicts
			{
				...
				{
					// the next is already in OPEN, cannot just pushNode.
					bool add_to_focal = false;  // check if it was above the focal bound before and now below (thus need to be inserted)
					bool update_in_focal = false;  // check if it was inside the focal and needs to be updated (because f-val changed)
					bool update_open = false;

					// Can next be inside the FOCAL?
					if ((next_g_val + next_h_val) <= w * min_f_val)
					{  // if the new f-val qualify to be in FOCAL
						// Yes, it can be.
						// Was it not in the FOCAL before?
						if (existing_next->getFVal() > w * min_f_val)
							// No, it was not in FOCAL,
					...
					// Do we need to update it in OPEN?
					if (existing_next->getFVal() > next_g_val + next_h_val)
						update_open = true; // Yes, because f-val decreased.
