...
// cat_size is given as the makespan (longest path)
void ConstraintTable::buildCAT(int agent, const vector<Path*>& paths, size_t cat_size)
{
	...
	// Mark cat by going through all the existing paths
	for (size_t ag = 0; ag < paths.size(); ag++)
	{
		// we don't want to mark for this agent or empty path.
		if (ag == agent || paths[ag] == nullptr)
			continue;

		// Now for this path, along the timestep
		for (size_t timestep = 0; timestep < paths[ag]->size(); timestep++)
		{
			// mark the location at this timestep occupied.
			cat[timestep][paths[ag]->at(timestep).location] = true;
		}
		// After the path reaches its goal, mark all after time as occupied.
...
// build the conflict avoidance table
// cat_size is given as the makespan (longest path)
void ConstraintTable::buildCATwithSemantic(int agent, const vector<Path*>& paths, size_t cat_size)
{

	cat_size = std::max(cat_size, (size_t)latest_timestep);
	cat.resize(cat_size, vector<bool>(map_size, false));

	// copy catTarget, which marks Target Conflicts
	catTarget.resize(cat_size, vector<bool>(map_size, false));
	// Mark cat by going through all the existing paths
	for (size_t ag = 0; ag < paths.size(); ag++)
	{
		// we don't want to mark for this agent or empty path.
		...
		for (size_t timestep = 0; timestep < paths[ag]->size(); timestep++)
		{
			// Is it target conflict? and do we want to exclude it?
			if (timestep== paths[ag]->size()-1)
				catTarget[timestep][paths[ag]->at(timestep).location] = true;
...
int ConstraintTable::getNumOfTargetConflictsForStep(size_t curr_id, size_t next_id, int next_timestep) const
{
	if (next_timestep >= (int)catTarget.size())
	{
		if (catTarget.back()[next_id])
			return 1;
		else
			return 0;
	}
	if (catTarget[next_timestep][next_id] ||
		(curr_id != next_id && catTarget[next_timestep - 1][next_id] && catTarget[next_timestep][curr_id]))
		return 1;
	else
		return 0;
}
...
// build the conflict avoidance table
// cat_size is given as the makespan (longest path)
void ConstraintTable::buildCATwithNumber(int agent, const vector<Path*>& paths, size_t cat_size, bool excludeTarget)
{
	cat_size = std::max(cat_size, (size_t)latest_timestep);
	catNum_.resize(cat_size, vector<int>(map_size, 0));
	// Mark cat by going through all the existing paths
	for (size_t ag = 0; ag < paths.size(); ag++)
	{
		// we don't want to mark for this agent or empty path.
		...
		{
			// Is it target conflict? and do we want to exclude it?
			if (excludeTarget && timestep== paths[ag]->size()-1)
				continue;
			// mark the location at this timestep occupied.
			(catNum_[timestep][paths[ag]->at(timestep).location])++;
		}
		// After the path reaches its goal, mark all after time as occupied.
		if (!excludeTarget)
		{
			int goal = paths[ag]->back().location;
			for (size_t timestep = paths[ag]->size(); timestep < cat_size; timestep++)
				(catNum_[timestep][goal])++;
		}
	}
}

int ConstraintTable::getNumOfConflictingAgentsForStep(size_t curr_id, size_t next_id, int next_timestep) const
{
	// if the next timestep is max, then return the last one
	if (next_timestep >= (int)catNum_.size())
	{
		return catNum_.back()[next_id];
	}
	// else return the number of vertex conflicts
	if (catNum_[next_timestep][next_id]>0)
		return catNum_[next_timestep][next_id];

	// else return the number of edge conflicts
	if (curr_id != next_id && catNum_[next_timestep - 1][next_id]>0 && catNum_[next_timestep][curr_id]>0)
		return catNum_[next_timestep - 1][next_id]+catNum_[next_timestep][curr_id];
}
